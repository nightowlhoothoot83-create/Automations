...
`config/saas-verification-contract.json` defines the six verified Raven Sharp products and the mandatory visual, functional, legal, backend, and billing surfaces. Backend health and Stripe test URLs remain configurable until discovered. Each capability advertised on public or pricing pages must map to a passing test and retained evidence. End-to-end checks must exercise API calls, core workflows, preview/rendering, output, save-and-reload persistence, and storage read/write using safe test data. A visual pass can never override any claim or functional failure, and billing verification is restricted to test mode or non-transactional inspection—never a live charge.
