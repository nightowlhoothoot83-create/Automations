...
export function validateSaasVerificationContract(contract) {
  ...
  if (architecture?.selectionMode !== 'independent-per-stage-after-benchmark' || architecture.allowDifferentProviders !== true || architecture.artworkGeneration?.status !== 'pending-owner-review' || architecture.previewGeneration?.status !== 'pending-owner-review' || fal?.clientPackage !== '@fal-ai/client' || fal.model !== 'FLUX 1.1 Pro' || fal.endpointId !== 'fal-ai/flux-pro/v1.1' || fal.draftEndpointId !== 'fal-ai/flux-1/schnell' || fal.credentialEnv !== 'FAL_KEY') throw new Error('POD split provider architecture is incomplete');
  const art = contract.podArtDirection;
  if (!art?.compileStructuredPrompt || art.finalArtworkModel !== 'fal-ai/flux-pro/v1.1' || art.draftOnlyModel !== 'fal-ai/flux-1/schnell' || art.minimumOverallReviewScore < 80 || !art.automaticFailure?.includes('generic-clip-art') || art.failedOutputAction !== 'reject-and-regenerate-with-targeted-correction') throw new Error('POD art-direction quality contract is incomplete');
